...
class A2CAgent:
    ...
    # c get_action(): get state, predict policy, select action with policy
    ...
    # 가치신경망을 업데이트하는 함수
    # c critic_optimizer(): returns "to be trained node" for value nn
    ...
    # c train_model(): updates polocy nn and value nn on each time step
    def train_model(self, state, action, reward, next_state, done):
        ...
        # 벨만 기대 방정식를 이용한 "어드벤티지"와 "업데이트 타깃"
        if done:
...
if __name__ == "__main__":
    ...
    # 액터-크리틱(A2C) 에이전트 생성
    # c agent: actor critic agent object
